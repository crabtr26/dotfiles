...
plugins=(aliases copybuffer copyfile copypath dirhistory docker fluxcd git gitfast git-auto-fetch git-commit helm history kubectl minikube poetry pre-commit python terraform tmux z zsh-interactive-cd zsh-autosuggestions zsh-syntax-highlighting)
...
# Add alias definitions from .aliases file
if [ -f ~/.aliases ]; then
    . ~/.aliases
fi
